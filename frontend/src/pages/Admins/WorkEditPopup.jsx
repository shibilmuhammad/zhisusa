...
const WorkEditPopup = ({ setLoadData , loadData , setShowEdit, categoryList ,loading ,rowID ,dataList}) => {
  ...
  const formData = {
    ...
    workingspaceNumber:useRef(null),
    floor: useRef(null),
  ...
  useEffect(()=>{
    ...
    formData.capacity.current.value = dataList[rowID].details.capacity
    formData.size.current.value = dataList[rowID].details.size
    formData.status.current.value = dataList[rowID].status
    formData.workingspaceNumber.current.value = dataList[rowID].details.location.workSpaceNumber
    ...
    formData.perHour.current.value = dataList[rowID].details.price.perHour
    formData.perDay.current.value =  dataList[rowID].details.price.perDay
  ...
  const validateForm = () => {
    ...
    } else if (formData.capacity.current.value.trim() ==='') {
      errors.capacity = "Capacity can't be empty!";
      formData.capacity.current.focus()
    } else if (formData.size.current.value.trim()==='') {
      errors.size = "Size can't be empty!";
      formData.size.current.focus()
    } else if (formData.workingspaceNumber.current.value.trim()==='') {
      errors.workingspaceNumber = "Room number can't be empty!";
      formData.workingspaceNumber.current.focus()
    ...
    } else if (formData.perHour.current.value.trim()==='') {
      errors.perHour = "Price per Hour can't be empty!";
      formData.perHour.current.focus()
    } else if (formData.perDay.current.value.trim()==='') {
      errors.perDay = "Price per Day can't be empty!";
      formData.perDay.current.focus()
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("/api/admin/editwork", data, {
        headers: {
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} class="  w-6/12  h-[80vh] relative">
        {loading ? (
          <div className="h-32 w-32 bg-red-700">
          ...
          <div class="w-full space-y-1">
            ...
            <div className="">
              <input
                ...
                name="capacity"
                onChange={handleChange}
                ref={formData.capacity}
                placeholder="In Square Feet (sq ft)"
                class={`px-2 border-gray-400 border-[.1px] w-full p-2 rounded-lg ${errMsg.capacity? 'border-red-500 focus:outline-red-400':'' } `}
              />
              {errMsg.capacity && (
        ...
        <div className="flex gap-5">
          <div class="w-full space-y-1">
            ...
            <div className="">
              <input
              ref={formData.workingspaceNumber}
                type="text"
                name="workingSpaceNumber"
                placeholder="eg:221B"
                onChange={handleChange}
                className={`px-2 border-gray-400 border-[.1px] w-full p-2 rounded-lg ${errMsg.workingspaceNumber ? 'border-red-500 focus:outline-red-400' : ''
              }`}
              />
              {errMsg.workingspaceNumber && (
                <span className="text-[10px] text-red-600 space-y-0">
                  {errMsg.workingspaceNumber}
                </span>
              )}
        ...
        <div className="flex gap-5 w-full items-center">
          ...
          <div className={`rounded-lg  border-[.1px] w-8/12 flex items-center ${errMsg.perHour || errMsg.perDay || errMsg.perMonth ? 'border-red-500 focus:outline-red-400':'border-gray-400'}`}>
            <div className="bg-gray-100 rounded-l-lg p-2 px-2 flex items-center justify-end">
              <span>Hour</span>
            </div>
            <span className="px-2">₹</span>
            <input
              className={`w-full px-2 py-1.5 h-full ${errMsg.perHour?'border-red-500 focus:outline-red-400' : ''}`}
              type="number"
              ref={formData.perHour}
              name="perHour"
              placeholder="eg:1500"
              onChange={handleChange}
              id=""
            ...
            <span className="px-2">₹</span>
            <input
               className={`w-full px-2 py-1.5 h-full ${errMsg.perDay?'border-red-500 focus:outline-red-400' : ''}`}
              type="number"
              placeholder="10,500"
              name="perDay"
              ref={formData.perDay}
